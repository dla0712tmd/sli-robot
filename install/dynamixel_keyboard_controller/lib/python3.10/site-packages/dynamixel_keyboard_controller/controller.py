...
TORQUE_ENABLE = 1
TORQUE_DISABLE = 0
DXL1_ID = 1  # 머리를 앞쪽으로 가게 한 상태에서 봤을때 아래 왼쪽 3,4,1,2
DXL2_ID = 2  # 아래 오른쪽
DXL3_ID = 3  # 위 왼쪽
DXL4_ID = 4  # 위 오른쪽
        ...
        try:
            while True:
                ...
                if key == 'q':    	#위로 올리기(3,4번 당기기)
                    ...
                    self.set_motor_speed(DXL1_ID, 300) #1:2비율 , 3:4비율도 안됨, 더 작은 가격으로 풀려야함4:5????
                    ...
                    self.set_motor_speed(DXL3_ID, 1624)
                    print("3 당기기\n")    
                elif key == 'A':	# 3 풀기
                    self.set_motor_speed(DXL3_ID, 600)
                    print("3 풀기\n")   
                
                elif key == 's':	# 4 당기기
                    self.set_motor_speed(DXL4_ID, 600)
                    print("4 당기기\n")    
                elif key == 'S':	# 4 풀기
                    self.set_motor_speed(DXL4_ID, 1624)
                    print("4 풀기\n")   
                    
                elif key == 'd':	# 1 당기기
                    self.set_motor_speed(DXL1_ID, 1624)
                    print("1 당기기\n")    
                    
                elif key == 'D':	# 1 풀기
                    self.set_motor_speed(DXL1_ID, 600)
                    print("1 풀기\n")   
                    
                elif key == 'f':	# 2 당기기
                    self.set_motor_speed(DXL2_ID, 600)
                    print("2 당기기\n")    
                elif key == 'F':	# 2 풀기
                    self.set_motor_speed(DXL2_ID, 1624)
                    print("2 풀기\n")   
